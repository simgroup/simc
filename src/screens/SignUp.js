  ...
  handleFieldChange(event, type) {
    switch (type) {
      ...
      case ("email"):
        ...
        break;
      // case ("phoneNo"):
      //   if (event.target.value.length === 10 && event.target.value.match(/^\d{10}$/)) {
      //     this.setState({ Mob_Number: event.target.value })
      //   }
      //   else {
      //     console.log("length", event.target.value.length, "matching", event.target.value.match(/^\d{10}$/))
      //   }
      //   break;
      case ("pass"):
...
//  SOSIAL LOGIN BUTTONS Handler STARTS

  componentClicked = () => console.log("clicked login")

  responseFacebook = (resp) => {
    this.props.history.push({ pathname: "/social", state: {email: resp.email} })
    console.log(resp)
  }


  componentGoogle = () => console.log("login for Google")

  responseGoogle = (res) => {
    localStorage.setItem("GoogleData", res)
    console.log("getting here ====>>>", res);
  } 
//  SOSIAL LOGIN BUTTONS Handler ENDS
      ...
      <div className="remove">
        ...
        <main className="main-cover">
          ...
          {/* ValidationResponse ENDS*/}
                        ...
                        <input type="text" id="emailAddress" name="cv cv emailAddress" placeholder="Email Address"
          ...
          {/* ValidationResponse */}
                        {this.state.emailStatus ?
                          this.responseMessage()
                          : null}
                        {this.state.emailLen == 0 ? this.blankResponseMessage() : null}
          {/* ValidationResponse ENDS*/}
          ...
          {/* ValidationResponse */}
                      ...
                      <div className="uppdte mt-4" style={{ position: "relative" }}>
                        <button type="submit">
                          Login with Facebook
                          <i className="fa fa-facebook" style={{ marginLeft: "12px", fontSize: "18px" }}></i>
                        </button>

                {/* SOCIAL LOGIN BUTTONS*/}
                        <FacebookLogin
                          appId="382181229120670"
                          autoLoad={false}
                      ...
                      <div className="uppdte mt-4">
                        <button type="submit" onClick={()=>{ToastsStore.error("Work in progress....")}}>
                          {/* Login with Google */}
                          Login with Google
                          <i className="fa fa-google-plus" style={{ marginLeft: "40px", fontSize: "20px" }}></i></button>
                        {/* <GoogleLogin
                          className="google-login-style"
                          clientId="658977310896-knrl3gka66fldh83dao2rhgbblmd4un9.apps.googleusercontent.com"
                          //buttonText="Login"
                          onClick={this.componentGoogle}
